...
function Level(plan) {
    ...
    for (var y = 0; y < this.height; y++) {
        ...
        for (var x = 0; x < this.width; x++) {
            ...
            if (Actor) this.actors.push(new Actor(new Vector(x, y), ch)); else if (ch == "x") fieldType = "wall"; else if (ch == "!") fieldType = "lava";
...
Player.prototype.moveX = function(step, level, keys) {
    ...
    if (obstacle) level.playerTouched(obstacle); else this.pos = newPos;
};
...
Player.prototype.act = function(step, level, keys) {
    ...
    if (level.isOnline) this.moves = keys;
